...
class Friendlist{
    ...
    public function agree(){
        ...
        $friend_id = input("post.friend_id",0);
        ...
        try{
            ...
            Db::table('flist')->where("id",$flist['id'])->update(['member'=>$flist['member'],'count'=>$flist['count'] + 1]);
            Db::table('pmessage')->insert(['user_id'=>$user_id,'goal_id'=>$friend_id,'content'=>config('hello')]);
            Db::commit();
        }catch (\Exception $e) {
            Db::rollback();
        ...
        //向双方发送通知
        $sql = "SELECT id,name,status,
                    (SELECT content FROM pmessage WHERE user_id=$user_id AND goal_id=$friend_id
                        ORDER BY create_time desc LIMIT 0,1) AS last_msg,
                    (SELECT create_time FROM pmessage WHERE user_id=$user_id AND goal_id=$friend_id
                        ORDER BY create_time desc LIMIT 0,1) AS last_time,
                    (SELECT count(id) FROM pmessage WHERE status=1 AND user_id=$user_id AND goal_id=$friend_id)
                        AS `count`
                FROM `user`
                WHERE id=$friend_id";
        $res = Db::query($sql)[0];
