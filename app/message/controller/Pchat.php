...
class Pchat{
    //获取私聊信息  get     goal_id     msg_id
    public function getMsg(){
        $user_id = session("user_id");
        $user_id = $user_id?$user_id:input("get.user_id");
        ...
        try{
            ...
            $sql = "SELECT (IF(user_id=$user_id,1,0)) AS status,(SELECT name FROM user WHERE pmessage.user_id=user.id)AS name,create_time,content,id,0 AS `type`
                    FROM pmessage 
        ...
        return ['friend_name'=>$user['name'],'msg'=>json_encode($res)];
    }
    //重置消息状态  post    goal_id
